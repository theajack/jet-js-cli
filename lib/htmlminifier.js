...
const lessMod=require('../lib/less')
var toCss=function(str){
  if(str.trim()===''){
      return '';
  }
  var root=lessMod.parse(str,()=>{})
  var imports={}//new lessMod.ImportManager('','','')
  var parseTree = new lessMod.ParseTree(root,imports);
...
function buildAttr(normalized, hasUnarySlash, options, isLast, uidAttr) {
  ...
  //修改.less 后缀名
  var _trim=attrFragment.trim();
  var _attr=_trim.substring(0,_trim.length-1).trim();
  if(_attr.substring(_attr.length-'.less'.length)==='.less'){
    attrFragment=attrFragment.substring(0,attrFragment.lastIndexOf('.less'))+'.css" '
  }
    ...
    if (key === 'caseSensitive') {
      ...
      options.minifyJS = function(text, inline) {
        ...
        if (result.error) {
          ...
          if(!options.useBabel){
            slog(chalk.red('The js code in the HTML file failed to compress. It may be that you are using es6 syntax or js syntax error. Please check your code'));
            console.log()
            console.log(chalk.red('If you want to use es6, configure jetConfig.buildJsWithBabel to true in packjson'));
            console.log()
          }else{
            slog(chalk.red('The js code in the HTML file failed to compress, which may be a js syntax error. Please check your code'));
            console.log()
          }
  ...
  new HTMLParser(value, {
    ...
    chars: function(text, prevTag, nextTag) {
      ...
      if (isExecutableScript(currentTag, currentAttrs)) {
        if(options.useBabel){
          var res=currentAttrs.find(function(attr){
            return attr.name=='jbabel'&&attr.value=='false'
          })
          if(!res){
            text=toEs5(text)
          }
      ...
      if (isStyleSheet(currentTag, currentAttrs)) {
        ...
        // console.log(text);
        // currentAttrs[0].value='aa.css'
        if(options.useLess){
          var res=currentAttrs.find(function(attr){
            return attr.name=='jless'&&attr.value=='false'
          })
          if(!res){
            text=toCss(text)
          }
    ...
    lessCallback:function(text,prevTag,nextTag){
